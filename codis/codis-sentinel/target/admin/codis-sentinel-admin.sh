...
CODIS_SERVER_PID_FILE=/tmp/redis-sentinel_6379.pid

CODIS_SERVER_LOG_FILE=/tmp/redis-sentinel_6379.log
CODIS_SERVER_DAEMON_FILE=$CODIS_LOG_DIR/redis-sentinel.out
...
start)
    ...
    ;;
start-foreground)
    $CODIS_SERVER_BIN ${CODIS_SERVER_CONF_FILE}
    ;;
stop)
    echo "stopping codis-sentinel ... "
    if [ ! -f "$CODIS_SERVER_PID_FILE" ]
    then
      echo "no codis-sentinel to stop (could not find file $CODIS_SERVER_PID_FILE)"
    else
